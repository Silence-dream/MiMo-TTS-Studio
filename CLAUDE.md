...
│   └── styles.ts            # 共享行内样式常量（iconBadgeStyle、glassCardStyle），减少渲染时对象创建
...
- **localStorage**：配置和历史元数据（`mimo_api_key`、`mimo_api_endpoint`、`mimo_tts_history`、`mimo_favorite_voices`、`mimo_voice_usage`、`theme`、`has_seen_guide`）。历史记录上限 20 条
...
并发控制为 3（`CONCURRENCY = 3`），使用 Promise.race 实现有限并发队列。每条结果直接写入合成历史（IndexedDB + localStorage）。批量过程支持中断（与单次共享 `abortControllerRef`）。

ZIP 打包是合成历史的「批量下载」特性（在 `HistoryList` 中实现，使用 JSZip），与批量合成本身无关。
...
- **Code Inspector**：`next.config.ts` 集成 `code-inspector-plugin`，开发模式下可在浏览器中按住组合键点击页面元素，跳转到 IDE 中对应源码位置。
